  ...
  return (
    <div ref={containerRef} className="relative rounded-lg overflow-hidden border border-border/50 bg-card/60">
      <div className="w-full aspect-[16/9] relative">
        <img src={slide.bg} alt={slide.title} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute inset-0 p-8 flex flex-col justify-between">
          ...
          <div className="max-w-2xl">
            ...
            {slide.contentType === "game" && slide.game === "rhythm" && (
              <div className="mt-4 bg-background/60 p-3 rounded"> <RhythmGame /> </div>
            )}
            {slide.contentType === "game" && slide.game === "unscramble" && (
              <div className="mt-4 bg-background/60 p-3 rounded"> <WordUnscramble answer="Imagination" /> </div>
            )}
          </div>
        </div>
      ...
      <div className="border-t border-border/30 p-3 overflow-x-auto">
        <div className="flex gap-2">
          {SLIDES.map((s, i) => (
            <button key={s.id} onClick={() => setIndex(i)} className={`w-36 flex-shrink-0 rounded-md overflow-hidden border ${i === index ? "ring-2 ring-primary" : ""}`}>
