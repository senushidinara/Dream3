  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-border/50 bg-card/40">
      <div className="relative w-full aspect-[16/9]">
        ...
        <div className="absolute inset-0"><Stardust intensity={36} color="230,220,255" /></div>

        {/* HUD */}
        <div className="absolute left-4 top-4 flex gap-2 text-xs">
          <div className="px-2 py-1 rounded bg-[rgba(0,0,0,0.55)] text-white border border-border/40">Fragments {game.fragments}</div>
          <div className="px-2 py-1 rounded bg-[rgba(0,0,0,0.55)] text-white border border-border/40">Shards {game.shards}</div>
          <div className="px-2 py-1 rounded bg-[rgba(0,0,0,0.55)] text-white border border-border/40">Keys {game.keys}</div>
        </div>
        ...
        {[
          ...
          >
            ...
            <span className="relative text-[11px] font-semibold">{d.label}</span>
          </button>
        ))}

        {/* Travel nodes to other realms */}
        <div className="absolute right-4 top-4 grid gap-2">
          <button onClick={() => travel("/islands")} className="px-2 py-1 rounded bg-primary/20 text-primary text-xs">Islands</button>
          <button onClick={() => travel("/gallery")} className="px-2 py-1 rounded bg-secondary text-secondary-foreground text-xs">Gallery</button>
          <button onClick={() => travel("/library")} className="px-2 py-1 rounded bg-muted text-xs">Library</button>
          <button onClick={() => travel("/constellation")} className="px-2 py-1 rounded bg-muted text-xs">Constellation</button>
        </div>

        {/* Collectibles */}
        <DreamFragment id="quest-frag-1" className="absolute left-[48%] top-[26%]" />
        <DreamFragment id="quest-frag-2" className="absolute left-[32%] top-[70%]" />
        <DreamFragment id="quest-frag-3" className="absolute left-[66%] top-[28%]" />
      </div>
      ...
      <Dialog open={open === "puzzle"} onOpenChange={(o) => !o && setOpen(null)}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
      ...
      <div className="p-3 text-xs text-muted-foreground">
        Goal: collect 3 fragments (left), win 1 shard (music), earn 1 key (puzzle). Travel anywhere from the panel on the right.
      </div>
    </div>
  );
}
