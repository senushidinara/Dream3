  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gradient-to-b from-background to-background/40">
      <div className="mx-auto max-w-7xl px-4 py-12">
        <h1 className="text-3xl font-extrabold tracking-tight">Dream Constellation</h1>
        <p className="text-muted-foreground mt-2">Your journey traced in stars. Nodes glow as your fragments, shards, and keys grow.</p>
        ...
        <div className="mt-8 rounded-2xl border border-border/50 bg-card/40 overflow-hidden">
          <svg viewBox="0 0 100 60" className="w-full aspect-[5/3] bg-[radial-gradient(ellipse_at_center,hsl(var(--primary)/.15),transparent_60%)]">
            {LINKS.map(([a, b], idx) => {
              ...
              return (
                <line key={idx} x1={A.x} y1={A.y} x2={B.x} y2={B.y} stroke="hsl(var(--primary))" strokeOpacity="0.3" strokeWidth="0.4" />
              );
            })}
            {STARS.map((s) => {
              ...
              return (
                <g key={s.id}>
                  <circle cx={s.x} cy={s.y} r={lit ? 2.6 : 1.8} fill={lit ? "hsl(var(--accent))" : "hsl(var(--muted-foreground))"} />
