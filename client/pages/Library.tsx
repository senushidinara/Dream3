  ...
  return (
    <div className="rounded-xl border border-border/50 p-4 bg-card/60">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {[{ id: "curiosity", label: "Curiosity" }, { id: "courage", label: "Courage" }, { id: "clarity", label: "Clarity" }, { id: "joy", label: "Joy" }].map((o) => (
          <button key={o.id} onClick={() => setChoice(o.id)} className={`px-3 py-1.5 rounded-md border ${choice === o.id ? "bg-primary text-primary-foreground border-transparent" : "bg-background hover:bg-muted/50"}`}>
            {o.label}
          </button>
        ))}
      </div>
      ...
      <div className="mt-4 text-right">
        <button onClick={onClose} className="px-3 py-1 rounded bg-muted">Close</button>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-[calc(100vh-64px)]">
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className="relative rounded-xl overflow-hidden" style={{ height: 320 }}>
          ...
          <div className="absolute inset-0 p-8 flex items-end">
            <div className="text-white">
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight">Memory Library</h1>
              <p className="text-white/95 mt-2 max-w-2xl">Solve puzzles woven from your memories. Some tomes require fragments from elsewhere.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-6">
          {BOOKS.map((b) => (
            <div key={b.id} className="rounded-xl border border-border/50 p-4 bg-card/60 relative">
              <div className="aspect-[4/3] rounded overflow-hidden bg-muted/10">
                <img src={b.img} alt={b.title} className="w-full h-full object-cover" />
              </div>
              <h4 className="mt-3 font-semibold">{b.title}</h4>
              <p className="text-sm text-muted-foreground">{b.type === "puzzle" ? "Open to play a puzzle." : "Reflect and choose a theme."}</p>
              <div className="mt-3 flex gap-2">
                <button onClick={() => setOpenBook(b.id)} className="px-3 py-1 rounded bg-primary/20 text-primary">Open</button>
                <DreamFragment id={`frag-lib-${b.id}`} className="absolute -top-2 -right-2" />
              </div>
            </div>
          ))}
        </div>
      ...
      {/* book modal */}
      {openBook && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70" onClick={() => setOpenBook(null)}>
          <div className="w-[min(900px,95%)] bg-card/90 rounded-lg p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start gap-4">
              <div className="w-1/2">
                <img src={BOOKS.find((x) => x.id === openBook)!.img} alt="book" className="w-full h-full object-cover rounded" />
              </div>
              <div className="flex-1">
                <h3 className="text-xl font-bold">{BOOKS.find((x) => x.id === openBook)!.title}</h3>
                <div className="mt-4">
                ...
                <div className="mt-4 text-right">
                  <button onClick={() => setOpenBook(null)} className="px-3 py-1 rounded bg-muted">Close</button>
                </div>
              </div>
            </div>
          </div>
